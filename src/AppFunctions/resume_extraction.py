...
def make_prediction(cleaned_text, model, tfidf_vectorizer):
    transformed_text = tfidf_vectorizer.transform([cleaned_text])
    predicted_category_number = model.predict(transformed_text)[0]
    
    with open('data/processed/label_encoder.pkl', 'rb') as f:
        label_encoder = pickle.load(f)
    
    predicted_category = label_encoder.inverse_transform([predicted_category_number])[0]
    return predicted_category

def load_model_and_vectorizer(model_dir='model evaluation'):
    model_path = None
    ...
    with open('data/processed/tfidf_vectorizer_categorization.pkl', "rb") as f:
